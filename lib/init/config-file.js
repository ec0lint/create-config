...
const debug = debugEsm("ec0lint:config-file");
...
async function writeJSConfigFile(config, filePath) {
    ...
    const ec0lintBin = path.join(cwd, "./node_modules/.bin/ec0lint");
    const result = spawn.sync(ec0lintBin, ["--fix", "--quiet", filePath], { encoding: "utf8" });
