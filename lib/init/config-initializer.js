...
import { Legacy } from "@ec0lint/ec0lintrc";
...
function getModulesList(config, installEc0Lint) {
    ...
    if (config.plugins) {
        for (const plugin of config.plugins) {
            const moduleName = naming.normalizePackageName(plugin, "ec0lint-plugin");

            modules[moduleName] = "latest";
        }
        if (config.plugins.includes("html")) {
            modules["@html-eslint/parser"] = "latest";
        }
    }
    if (config.extends) {
        ...
        for (const extend of extendList) {
            if (extend.startsWith("ec0lint:") || extend.startsWith("plugin:")) {
                continue;
            }
    ...
    if (installEc0Lint === false) {
        ...
        const installStatus = npmUtils.checkDevDeps(["ec0lint"]);
...
function promptUser() {
    ...
    return enquirer.prompt([
...

        {
            type: "select",
            name: "framework",
            message: "Do you use React?", // "Which framework does your project use?",
            initial: 0,
            choices: [
                { message: "Yes", name: "react" },
...

        {
            type: "select",
            name: "framework",
            message: "Do you use HTML?", // "Which framework does your project use?",
            initial: 0,
            choices: [
                { message: "Yes", name: "html" },
                { message: "No", name: "none" },
                // { message: "Vue.js", name: "vue" },
                // { message: "Angular", name: "angular" },
